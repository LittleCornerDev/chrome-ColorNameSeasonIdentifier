...
const CnsiContent = {
  ...
  addListeners: function () {
    ...
    document.body.addEventListener("mousemove", CnsiContent.handleMouseMove);
  ...
  removeListeners: function () {
    ...
    document.body.removeEventListener("mousemove", CnsiContent.handleMouseMove);
  ...
  handleClick: async function (event: MouseEvent) {
    ...
    chrome.runtime.onConnect.addListener(async (port) => {
      ...
      const response = await chrome?.runtime?.sendMessage({ event: "clicked" });
  ...
  handleResize: async function (event: UIEvent) {
    ...
    chrome.runtime.onConnect.addListener(async (port) => {
      ...
      const response = await chrome?.runtime?.sendMessage({ event: "resized" });
  ...
  displayNameInfo: function (originHexCode: ColorValueHexWithHash) {
    ...
    if (nameData) {
      if (nameHexSpan && nameData.hex) nameHexSpan.innerHTML = nameData.hex;
      ...
      if (nameNamesList && nameData.names) {
        nameData.names.forEach(function (data, i) {
          nameNamesList.innerHTML +=
            "<li>" +
            utils.getColorNameDisplay(data.name, data.source, data.number) +
            "</li>";
        });
      }
    }
    ...
    if (canvas && nameData.hex) utils.fillCanvasWithHex(canvas, nameData.hex);
  },
  ...
  displaySeasonInfo: function (originHexCode: ColorValueHexWithHash) {
    ...
    if (seasonData) {
      ...
      const seasonsSection = document?.getElementById(CnsiContent.id.seasons);
      ...
      if (seasonsBadList && seasonData.bad) {
        seasonData.bad.forEach(function (data, i) {
          seasonsBadList.innerHTML +=
            "<li>" +
            utils.getColorSeasonDisplay(data.season, data.seasonType) +
            "</li>";
        });
      }
    }
  ...
  setImageData: function () {
    ...
    rgbaKeys.forEach((component, componentIndex) => {
      ...
      const span = document
        ?.getElementById(
          CnsiContent.id[
            ("originRgba" + component.toUpperCase()) as StyleClassNameKey
          ],
        )
        ?.querySelector("span");
    ...
    hsvKeys.forEach((component, componentIndex) => {
      ...
      const span = document
        ?.getElementById(
          CnsiContent.id[
            ("originHsv" + component.toUpperCase()) as StyleClassNameKey
          ],
        )
        ?.querySelector("span");
      if (span) {
        ...
        span.innerHTML += utils.getHSVdescription(component, componentValue);
      }
    });

    if (hexSpan) {
      // Use origin hex code to look for possible name name
      CnsiContent.displayNameInfo(hexSpan.innerHTML as ColorValueHexWithHash);

      //Use origin hex code to look for possible wardrobe seasons
      CnsiContent.displaySeasonInfo(hexSpan.innerHTML as ColorValueHexWithHash);
    }
  },
};

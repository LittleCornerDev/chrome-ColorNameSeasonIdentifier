...
CNSI.utils = {
	...
	getColorNameDisplay: function (name, source, number = null) {
		...
		var display = name + " [";
		display += '<a href="' + sourceUrls[ source ] + '" target="_blank">' + source + '</a>';
	...
	getColorSeasonDisplay: function (season, seasonType, colorType = null) {
		...
		var display = CNSI.utils.capitalizeFirstLetter(seasonType) + " " + CNSI.utils.capitalizeFirstLetter(season);
		if (colorType != null) {
	...
	RGBtoHCL: function (r, g, b) {
		let Y0 = 100, gamma = 3, Al = 1.4456, Ach_inc = 0.16;

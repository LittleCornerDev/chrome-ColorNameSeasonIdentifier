...
CNSI.content = {
	...
	createIds: function() {
		CNSI.content.ids.forEach(function(key) { 
			CNSI.content.id[key] = CNSI.content.id.main + CNSI.utils.capitalizeFirstLetter(key);
	...
	// Populated further in initIds().
	id: {
		main: "cnsi"  //Must name ids in content.css
	},
	
	// List of id keys for `id` population in initIds().
	...
	*/
	...
	getHTML: function() {
		return `<!-- BEGIN: Origin -->
			<section id="cnsiOrigin">
				<h5>Original Color</h5>
				<img/>
				<canvas></canvas>
				<div id="cnsiOriginHex">
					<label>Hex Code</label><span></span>
				</div>
				<div id="cnsiOriginRGBA">
					<div id="cnsiOriginRGBAr">
						<label>R</label><span></span>
					</div>
					<div id="cnsiOriginRGBAg">
						<label>G</label><span></span>
					</div>
					<div id="cnsiOriginRGBAb">
						<label>B</label><span></span>
					</div>
					<div id="cnsiOriginRGBAa">
						<label>A</label><span></span>
					</div>
				</div>
				<!--<div id="cnsiOriginHCL">
					<div id="cnsiOriginHCLh">
						<label>H</label><span></span>
					</div>
					<div id="cnsiOriginHCLc">
						<label>C</label><span></span>
					</div>
					<div id="cnsiOriginHCLl">
						<label>L</label><span></span>
					</div>
				</div>
				<div id="cnsiOriginHSL">
					<div id="cnsiOriginHSLh">
						<label>H</label><span></span>
					</div>
					<div id="cnsiOriginHSLs">
						<label>S</label><span></span>
					</div>
					<div id="cnsiOriginHSLl">
						<label>L</label><span></span>
					</div>
				</div>-->
				<div id="cnsiOriginHSV">
					<div id="cnsiOriginHSVh">
						<label>H</label><span></span>
					</div>
					<div id="cnsiOriginHSVs">
						<label>S</label><span></span>
					</div>
					<div id="cnsiOriginHSVv">
			...
			<section id="cnsiName">
				...
				<div id="cnsiNameExact">
					<label>Exact Match</label><span></span>
				</div>
				<div id="cnsiNameNames">
	...
	setImageData: function() {
		...
		rgbaKeys.forEach(function(component, componentIndex) {
			...
			var span = document.getElementById(CNSI.content.id["originRGBA"+component]).querySelector("span");
		...
		hclKeys.forEach(function(component, componentIndex) {
			...
			var span = document.getElementById(CNSI.content.id["originHCL"+component]).querySelector("span");
		...
		/*hslKeys.forEach(function(component, componentIndex) {
			...
			var span = document.getElementById(CNSI.content.id["originHSL"+component]).querySelector("span");
			if (component == "h") {
				span.innerHTML = componentValue;
			} else {
				span.innerHTML = componentValue + "%";
		...
		hsvKeys.forEach(function(component, componentIndex) {
			...
			var span = document.getElementById(CNSI.content.id["originHSV"+component]).querySelector("span");
			if (component == "h") {
				span.innerHTML = componentValue + "°";
			} else {
				span.innerHTML = componentValue + "%";
